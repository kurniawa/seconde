...
<main class="">
    ...
    <div class="p-2">
        ...
        <div class="font-bold text-2xl"><span>Rp. </span>{{ number_format((int)$item->harga * 100,2,',','.') }}</div>
        ...
        <div class="mt-12">
            @if (Auth::user())
            <form action="{{ route('items.mau', $item->id) }}" method="POST">
                @csrf
                <button type="submit" class="py-2 bg-emerald-400 rounded w-full text-white flex items-center justify-center gap-1">
                    <span>Saya mau ini</span>
                </button>
            </form>
            @else
            <button id="toggle-button" type="button" class="py-2 border-2 border-emerald-400 rounded w-full text-emerald-500 font-bold flex items-center justify-center gap-1" onclick="toggle_light(this.id, 'form-konfirmasi', [], ['bg-emerald-200'], 'block')">
                <span>Saya mau ini</span>
            </button>
            <div id="form-konfirmasi" class="border-2 bg-emerald-50 rounded p-2 mt-2 hidden">
                ...
                <form action="{{ route('items.mau', $item->id) }}" method="POST" class="mt-2">
                    @csrf
                    <div>
        ...
        <div class="grid grid-cols-2 gap-2 mt-2">
            <form action="{{ route('items.edit', $item->id) }}" method="GET">
                <button type="submit" class="py-2 bg-slate-400 rounded w-full text-white flex items-center justify-center gap-1">
                    <span>Edit</span>
            ...
            <form action="{{ route('items.delete', $item->id) }}" method="GET" onsubmit="return confirm('Anda yakin ingin menghapus barang ini?')">
                @csrf
                <button type="submit" class="py-2 bg-pink-500 rounded w-full text-white flex items-center justify-center gap-1">
                    <span>Hapus</span>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
